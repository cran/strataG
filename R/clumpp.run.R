...
#' 
#' @param sr result from \code{\link{structure.run}} or folder name containing STRUCTURE output files.
#' @param k choice of \emph{k} in \code{sr} to combine.
#' @param align.algorithm algorithm to be used for aligning the runs. Can be "full.search", "greedy", or "large.k".
#' @param sim.stat pairwise matrix similarity statistic to be used. Can be "g" or "g.prime".
#' @param greedy.option input order of runs to be tested. Required if \code{align.algorithm} is "greedy" or "large.k".
#'   Valid choices are:
#'   \tabular{ll}{
#'     \code{all} \tab test all possible input orders of runs (note that this option increases the run-time 
#'       sub-stantially unless R is small).\cr
#'     \code{ran.order} \tab test a specified number of random input orders of runs set by the \code{repeats} parameter.\cr
#'   }
#' @param repeats the number of input orders of runs to be tested. Only used if 
#'   \code{align.algorithm} is "greedy" or "large.k", and \code{greedy.option} is "ran.order".
#' @param order.by.run permute the clusters according to the cluster order of a specific run. 
#'   Set this parameter to a number from 1 to the number of runs in \code{sr}.
...
#' 
#' @references Available at \url{http://web.stanford.edu/group/rosenberglab/clumppDownload.html}
#' 
#' @seealso \code{\link{structure}}

clumpp.run <- function(sr, k, align.algorithm = "greedy", sim.stat = "g",
  ...
  param.file <- ifelse(is.null(label), "paramfile", paste(label, "paramfile", sep = "_"))
  ind.file <- ifelse(is.null(label), "indfile", paste(label, "indfile", sep = "_"))
  permutation.file <- ifelse(is.null(label), "permutationfile", paste(label, "permutationfile", sep = "_"))
  out.file <- ifelse(is.null(label), "outfile", paste(label, "outfile", sep = "_"))
  misc.file <- ifelse(is.null(label), "miscfile", paste(label, "miscfile", sep = "_"))
  
  eq.k <- sapply(sr, function(x) x$summary["k"] == k)
  if(sum(eq.k) == 0) stop(paste("no entries for k =", k, "found in 'sr'."))
  sr <- sr[eq.k]
  
  align.algorithm <- switch(align.algorithm, full.search = 1, greedy = 2, large.k = 3)
  ...
  greedy.params <- if(align.algorithm != 1) {
    ...
    if(greedy.option != 1) {
      if(is.null(repeats)) stop("'repeats' must be specified if 'greedy.options' is 'greedy' or 'large.k'.")
      gp <- c(gp, paste("REPEATS", repeats), paste("PERMUTATIONFILE", permutation.file))
      # write permutationfile
      perm.mat <- t(sapply(1:repeats, function(i) sample(1:length(sr))))
      write.table(perm.mat, file = permutation.file, quote = FALSE, sep = " ", row.names = FALSE, col.names = FALSE)
    }
    gp
  } else NULL
  ...
  write.table(q.mat.df, file = ind.file, quote = FALSE, sep = " ", row.names = FALSE, col.names = FALSE)
  ...
  if(!file.exists(out.file)) stop(paste("CLUMPP exited without creating output file, '", out.file, "'.", sep = ""))
  out.txt <- scan(out.file, "character", sep = "\n", quiet = TRUE)
  q.mat <- structure.parse.q.mat(out.txt, pops)
